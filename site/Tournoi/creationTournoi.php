<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            $sql = $bdd->prepare("INSERT INTO Tournoi (nomJeu, dateDebut, dateFin, idJeu, nbTeam, descTournoi, createur) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $sql->execute([$nom, $dateDebut, $dateFin, $idJeu, $nbTeam, $descTournoi, $idCreateur]);
...
<form method="POST">
    ...
    <div class="field">
        ...
        <select name="idJeu" required>
            ...
            <?php 
                $reqJeu = "SELECT idJeu, nom FROM Jeu ORDER BY nom ASC";
                $resultReqJeu = $bdd->query($reqJeu);

                if ($resultReqJeu->rowCount() > 0) {
                    while ($jeu = $resultReqJeu->fetch()) {
                        echo '<option value="' . $jeu['idJeu'] . '">' . $jeu['nom'] . '</option>';
                    }
                }
            ?>
        </select>

...
<main class="mx-auto max-w-6xl px-5 py-10 flex-1">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <h2 class="text-xl font-bold text-white">Gestion des Tournois</h2>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <?php while ($tournoi = $reqTournois->fetch()): ?>
                    <tr class="border-b border-slate-800">
                        <td class="py-3"><?php echo htmlspecialchars($tournoi['nomTournoi']); ?></td>
                        <td class="py-3"><?php echo htmlspecialchars($tournoi['nomJeu']); ?></td>
                        <td class="py-3"><?php echo htmlspecialchars($tournoi['dateDebut']); ?></td>
                        <td class="py-3"><?php echo htmlspecialchars($tournoi['dateFin']); ?></td>
                        <td class="py-3"><?php echo htmlspecialchars($tournoi['statut']); ?></td>
                        <td class="py-3">
                            <?php if ($tournoi['statut'] === 'Actif'): ?>
                                <a href="?idTournoi=<?php echo $tournoi['idTournoi']; ?>&action=ban" class="text-red-400">Bannir</a>
                            <?php else: ?>
                                <a href="?idTournoi=<?php echo $tournoi['idTournoi']; ?>&action=unban" class="text-green-400">Débannir</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endwhile; ?>
